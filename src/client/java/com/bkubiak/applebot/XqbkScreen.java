...
public class XqbkScreen extends Screen {
    ...
    private final AppleBotClient bot;
    ...
    public XqbkScreen(AppleBotClient bot) {
    ...
    protected void init() {
        ...
        int contentHeight = headerH + tableHeadH + (rowH * 6) + gap + inputHeight + gap + inputHeight + gap + layoutBtnHeight + gap + socialH;
        ...
        this.addDrawableChild(new GreenButton(
            ...
            "Storage: " + (bot.isStorageMode() ? "ON" : "OFF"), button -> {
                ...
                // Note: GreenButton doesn't support dynamic color update easily without recreating, 
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, Text.of("\u2022 " + statusText), panelX + panelW - 95, headerY, statusColor);

        // Profile line
        String playerName = client.player != null ? client.player.getName().getString() : "---";
        context.drawTextWithShadow(this.textRenderer, Text.of(bot.t("profile") + ": " + playerName), panelX + 15, headerY + (isCompact ? 14 : 18), COLOR_GRAY);

        // Table header
        int tableHeaderY = panelY + headerH;
        context.drawTextWithShadow(this.textRenderer, Text.of("#"), panelX + 15, tableHeaderY, 0xFF666666);
        context.drawTextWithShadow(this.textRenderer, Text.of(bot.t("function")), panelX + 35, tableHeaderY, 0xFF666666);
        context.drawTextWithShadow(this.textRenderer, Text.of(bot.t("status")), panelX + panelW - 80, tableHeaderY, 0xFF666666);
        ...
        context.drawTextWithShadow(this.textRenderer, Text.of(bot.t("command") + ":"), panelX + 15, textY, COLOR_GRAY);

        // Render widgets (Buttons & Fields)
    ...
    private void drawTableRow(DrawContext context, int num, String label, String value, int labelColor, int valueColor, int y) {
    ...
    private static class GreenButton extends ButtonWidget {
        ...
        protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
            ...
            context.drawTextWithShadow(net.minecraft.client.MinecraftClient.getInstance().textRenderer, this.getMessage(), textX, textY, textColor);
        }
    }
}
