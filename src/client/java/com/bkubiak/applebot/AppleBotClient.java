...
public class AppleBotClient implements ClientModInitializer {
    ...
    public void setTickDelay(int v) { tickDelay = v; }
    
    // Repair Mode: 0=OFF, 1=COMMAND, 2=CRAFT
    public int getRepairMode() { return repairMode; }
    public void setRepairMode(int v) { repairMode = v; }
    public void cycleRepairMode() { repairMode = (repairMode + 1) % 3; }
    ...
    public boolean isAutoRepair() { return repairMode == 1; }
    public void setAutoRepair(boolean v) { if (v) repairMode = 1; else if (repairMode == 1) repairMode = 0; }
    public boolean isFreeRepair() { return repairMode == 2; }
    public void setFreeRepair(boolean v) { if (v) repairMode = 2; else if (repairMode == 2) repairMode = 0; }
    ...
    public void setStorageMode(boolean v) { storageMode = v; }
    public int getStorageCycles() { return storageCyclesLimit; }
    public void setStorageCycles(int v) { storageCyclesLimit = v; }
    public void cycleStorageCycles() {
        if (storageCyclesLimit == 20) storageCyclesLimit = 100;
        else if (storageCyclesLimit == 100) storageCyclesLimit = 500;
        else if (storageCyclesLimit == 500) storageCyclesLimit = 1000;
        else if (storageCyclesLimit == 1000) storageCyclesLimit = 2000;
        else storageCyclesLimit = 20;
    }
    
    public String getRepairCommand() { return repairCommand; }
    public void setRepairCommand(String cmd) { repairCommand = cmd; }
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("xqbk")
                ...
                .then(ClientCommandManager.literal("gui")
                    .executes(context -> {
                        // Open GUI logic needs to run on client thread safely
                ...
                .then(ClientCommandManager.literal("config")
                    .then(ClientCommandManager.literal("delay")
                        .then(ClientCommandManager.argument("ticks", IntegerArgumentType.integer(1, 20))
                            .executes(context -> {
                                ...
                                tickDelay = ticks;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §7Delay: §a" + ticks + "t"));
                                return 1;
                            })
                        )
                    )
                    .then(ClientCommandManager.literal("repairmode")
                        .then(ClientCommandManager.literal("off")
                            .executes(context -> {
                                repairMode = 0;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §7Repair Mode: §cOFF"));
                                return 1;
                            }))
                        .then(ClientCommandManager.literal("command")
                            .executes(context -> {
                                repairMode = 1;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §7Repair Mode: §aCOMMAND"));
                                return 1;
                            }))
                        .then(ClientCommandManager.literal("craft")
                            .executes(context -> {
                                repairMode = 2;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §7Repair Mode: §aCRAFT"));
                                return 1;
                            }))
                    )
                    .then(ClientCommandManager.literal("repaircmd")
                        .then(ClientCommandManager.argument("command", StringArgumentType.word())
                            .executes(context -> {
                                ...
                                repairCommand = cmd;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §7" + t("command") + ": §a/" + cmd));
                                return 1;
                            })
                        )
                    )
                    ...
                    .then(ClientCommandManager.literal("autoeat")
                        .then(ClientCommandManager.literal("on")
                            .executes(context -> {
                                autoEat = true;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §aAuto-Eat: ON"));
                                return 1;
                            }))
                        .then(ClientCommandManager.literal("off")
                            .executes(context -> {
                                autoEat = false;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §cAuto-Eat: OFF"));
                                return 1;
                            }))
                    )
                    .then(ClientCommandManager.literal("storage")
                        .then(ClientCommandManager.literal("off")
                            .executes(context -> {
                                storageMode = false;
                                context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §cStorage: OFF"));
                                return 1;
                            }))
                        .then(ClientCommandManager.literal("on")
                             .executes(context -> {
                                 storageMode = true;
                                 context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §aStorage: ON (Default: " + storageCyclesLimit + ")"));
                                 return 1;
                             })
                             .then(ClientCommandManager.argument("cycles", IntegerArgumentType.integer(10))
                                 .executes(context -> {
                                     storageMode = true;
                                     storageCyclesLimit = IntegerArgumentType.getInteger(context, "cycles");
                                     context.getSource().sendFeedback(Text.of("§8[§2xQBK§8] §aStorage: ON (Cycles: " + storageCyclesLimit + ")"));
                                     return 1;
                                 }))
                        )
                    )
        ...
        source.sendFeedback(
            Text.literal("\uD83D\uDCBB GitHub")
                .setStyle(Style.EMPTY
                    ...
                    .withUnderline(true)
                    .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://github.com/xqbkubiak")))
            .append(Text.literal("  §8|  ").setStyle(Style.EMPTY))
            .append(Text.literal("\uD83D\uDCAC Discord")
                .setStyle(Style.EMPTY
                    .withColor(Formatting.WHITE)
                    .withUnderline(true)
                    .withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://discord.com/invite/getnotify")))));
        source.sendFeedback(Text.of("§8§m                                                  "));
    }
    ...
    private void performAction(MinecraftClient client) {
        ...
        if (state == 0 && repairMode > 0) {
             ...
             if (needsAction) {
                 if (repairMode == 2) {
                     ...
                     if (stack.isEmpty()) performCrafting(client);
        ...
        switch (state) {
            case 0: // Select Slot (Rotation)
                ...
                slotIndex++;
                if (slotIndex > 8 || slotIndex < minSlot) slotIndex = minSlot;
    ...
    private void performDump(MinecraftClient client) {
        dumpTimer++;
        if (dumpTimer < 5) return; // Wait a bit between steps
        ...
        switch (dumpState) {
            ...
            case 2: // Move Apples
                if (client.currentScreen instanceof net.minecraft.client.gui.screen.ingame.GenericContainerScreen) {
                    ...
                    boolean moved = false;
                    // Lower inventory slots (usually 27-62 for single chest + player inv, or similar offset)
    ...
    private void refillHotbar(MinecraftClient client) {
        ...
        for (int hotbarIndex = 2; hotbarIndex <= 8; hotbarIndex++) {
            ...
            if (hotbarStack.isEmpty()) {
                // Find leaves in main inventory (slots 9-35 in inventory = slots 9-35 in screen handler)
                for (int invSlot = 9; invSlot < 36; invSlot++) {
                    ...
                    if (isLeaves(invStack)) {
                        ...
                        try {
                            ...
                            client.interactionManager.clickSlot(syncId, screenInvSlot, 0, SlotActionType.PICKUP, client.player);
                            // Put in hotbar
                            client.interactionManager.clickSlot(syncId, screenHotbarSlot, 0, SlotActionType.PICKUP, client.player);
                            
                            client.player.sendMessage(Text.of("§8[§2xQBK§8] §a" + (language.equals("pl") ? "Uzupelnilem slot " : "Refilled slot ") + (hotbarIndex + 1)), true);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
